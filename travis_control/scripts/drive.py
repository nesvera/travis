...
class Control:
    ...
    def routine(self):
        ...
        while True:
            ...
            for sign in self.signs_detected:

                # stop the car if there is a traffic sign near
                if sign.distance < float(self.SIGN_DISTANCE_TO_STOP):
                    self.stop()
                    self.current_state = self.FSM_CROSS

                if sign.id == self.SIGN_ID_NO_ENTRY:
                    pass

                elif sign.id == self.SIGN_ID_DEAD_END:
                    pass

                elif sign.id == self.SIGN_ID_RIGHT:
                    pass

                elif sign.id == self.SIGN_ID_LEFT:
                    pass

                elif sign.id == self.SIGN_ID_FORWARD:
                    pass

                elif sign.id == self.SIGN_ID_STOP:
                    pass
    ...
    def turn(self, direction):
        ...
        self.ackermann_cmd.speed = 0
        self.ackermann_cmd.steering_angle = direction
    ...
    def sign_detection_update(self, data):
        ...
        self.signs_detected = data.sign_detected
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/travis/sign_detected', SignsDetected, sign_detection_callback, queue_size=1)
