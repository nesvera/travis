...
class ArucoTaura:
	...
	def feed(self,frame):
		...
		if self.ids.__class__!=None.__class__: self.existID = True
		else: self.existID = False
	...
	def findPosition(self,idselected):
		...
		if self.existID:
			if idselected in self.ids:
				if self.estimated == False:
					self.estimateArucoPose()
				indexofid = np.nonzero(self.ids == idselected)[0][0]
				x = (self.atvec[indexofid])[0][0][0]
				y = (self.atvec[indexofid])[0][0][1]
				z = (self.atvec[indexofid])[0][0][2]
	...
	def estimateArucoPose(self): #Function used by other one
		...
		if self.existID:
			...
			for x in xrange(self.n):
				self.avec[x],self.atvec[x],_ = aruco.estimatePoseSingleMarkers(self.corner[x], self.arucolen, self.newcameraMatrix, self.dist)
